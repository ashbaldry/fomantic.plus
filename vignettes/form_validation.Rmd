...
Using the [form validation example](https://github.com/ashbaldry/fomantic.plus/inst/examples/form_validation), we can see the following in the UI:
...
![](invalid_input.png){alt="Screenshot of app, with error message displayed in a message box under the form" width="500"}
...
![](invalid_inline_input.png){alt="Screenshot of app, with error message displayed as a tooltip under the inputs" width="500"}
...
- `"checked"` - Checks that a checbox input has been checked
...
- `"is", "not", "isExactly", "notExactly"` - Matches input exactly against a fixed string
  - When `"Exactly"` is included, the term is case sensitive
